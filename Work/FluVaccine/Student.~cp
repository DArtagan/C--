...
bool Student::catchFlu( const int locationCounts[] )
{
    ...
    double sigma_p  = locationCounts[Student::getLocation()] * infectionProb;  // Chance of someone being infected increases the more infectious people are in that area
...
{
    const string statusNames[STAGES] = {"Infectious", "Symptomatic", "Home", "Cured", "Uninfected"};  // The names of the flu stages
...
int Student::getDuration() const
{
    ...
    const int DURATIONS[] = { 52, 32, 48, 0, 0 };  // Duration for each flu stage
